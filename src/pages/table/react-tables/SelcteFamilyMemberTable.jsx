...
//   const getAllVoters = () => {
//     axios
//       .get(`${base_url}/api/surve/searchVotter?address=true&lastNameFilter=${surname}&nameFilter=${voterName}&page=${currentPage}`)
//       .then((resp) => {
...
const SeleteFamilyMemberTable = ({handelSetData,handleFamilyModal}) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [inputPage, setInputPage] = useState();
  const [allVoter, setAllVoter] = useState([]);
